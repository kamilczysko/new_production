 ...
 * Window - Preferences - Java - Code Style - Code Templates
 */
public class PrintContainerBarCode extends PrintLabel {
	 ...
	 * @param yLoc
	 */
	public PrintContainerBarCode(int xLoc, int yLoc, String [][] printData) {
		...
		fl.setHgap(50);
		JLabel lbl = new JLabel("Ilosc kopii:");
		bp.add(lbl);
		bp.add(getCopyNbJTF());
			...
			if (lop > 0) {
				...
				for (int i = 0; i < lnb; i++) {
					...
					for (int j = 0; j < rowLbNb; j++) {
						...
						g2d.setFont(etf);
						layout = new TextLayout(printData[lbi][0], etf, frc);	//indeks katalogowy
						dm = (int) Math.round((etW - layout.getBounds().getWidth()) / 2.0);
						g2d.drawString(printData[lbi][0], dm + lm + j * etW, 17 + gm + i * etH);
...
//						g2d.drawString(printData[lbi][2], dm + lm + j * etW, 26 + gm + i * etH);
						...
						g2d.setFont(etf);
						layout = new TextLayout(printData[lbi][0], etf, frc);	//kod paskowy
						dm = (int) Math.round((etW - layout.getBounds().getWidth()) / 2.0);
						g2d.drawString(printData[lbi][0], dm + lm + j * etW, 82 + gm + i * etH);
						etf = Font.decode("Code 128-PLAIN-44");
						g2d.setFont(etf);
						kodp = code128(printData[lbi][0]);//TODO id artykulu
			...
			if (formatka == 8 && page < printData.length) {	//drukarka Godex
				...
				g2d.setFont(etf);
				layout = new TextLayout(printData[page][0], etf, frc);	//indeks katalogowy
				dm = (int) Math.round((etW - layout.getBounds().getWidth()) / 2.0);
				g2d.drawString(printData[page][0], dm + lm, 35 + gm);
				layout = new TextLayout(printData[page][0], etf, frc);	//indeks handlowy
				dm = (int) Math.round((etW - layout.getBounds().getWidth()) / 2.0);
				g2d.drawString(printData[page][0], dm + lm, 59 + gm);
				etf = Font.decode("Verdana-PLAIN-10");
				g2d.setFont(etf);
				layout = new TextLayout(printData[page][0], etf, frc);	//kod paskowy
				dm = (int) Math.round((etW - layout.getBounds().getWidth()) / 2.0);
				g2d.drawString(printData[page][0], dm + lm, 135 + gm);
//				etf = Font.decode("Free 3 of 9 Extended-PLAIN-32");
				etf = Font.decode("Code 128-PLAIN-52");
				g2d.setFont(etf);
				kodp = code128(printData[page][0]);//
//				kodp = "*" + printData[page][0] + "*";
				layout = new TextLayout(kodp, etf, frc);
				dm = (int) Math.round((etW - layout.getBounds().getWidth()) / 2.0);
...
//						g2d.drawImage(logos.getImage(),skalowanie,logos.getImageObserver());
						Font etf = Font.decode("Impact-PLAIN-80");
						g2d.setFont(etf);
						FontRenderContext frc = g2d.getFontRenderContext();
						TextLayout layout = new TextLayout(printData[page][0], etf, frc);
						int dm = (int) Math.round((etW - layout.getBounds().getWidth()) / 2.0);
						g2d.drawString(printData[page][0], dm, 80);
						etf = Font.decode("Code 128-PLAIN-40");
						g2d.setFont(etf);
						kodp = code128(printData[page][0]);//
						layout = new TextLayout(kodp, etf, frc);
						dm = (int) Math.round((etW - layout.getBounds().getWidth()) / 2.0);
						g2d.drawString(kodp, dm, 120);
//						layout = new TextLayout(printData[page][0], etf, frc);
//						dm = (int) Math.round((etW - layout.getBounds().getWidth()) / 2.0);
//						g2d.drawString(printData[page][0], dm, 155);
//						etf = Font.decode("Verdana-PLAIN-14");
//						g2d.setFont(etf);
//						layout = new TextLayout(printData[page][0], etf, frc);
//						dm = (int) Math.round((etW - layout.getBounds().getWidth()) / 2.0);
//						g2d.drawString(printData[page][0], dm, 270);
////						etf = Font.decode("Free 3 of 9 Extended-PLAIN-60");
//						etf = Font.decode("Code 128-PLAIN-50");
//						g2d.setFont(etf);
...
//							g2d.drawImage(logos.getImage(),skalowanie,logos.getImageObserver());
							...
							layout = new TextLayout(printData[page][0], etf, frc);
							dm = (int) Math.round((etW - layout.getBounds().getWidth()) / 2.0);
							g2d.drawString(printData[page][0], dm, etH + 135);
							layout = new TextLayout(printData[page][0], etf, frc);
							dm = (int) Math.round((etW - layout.getBounds().getWidth()) / 2.0);
							g2d.drawString(printData[page][0], dm, etH + 155);
							etf = Font.decode("Verdana-PLAIN-14");
							g2d.setFont(etf);
							layout = new TextLayout(printData[page][0], etf, frc);
							dm = (int) Math.round((etW - layout.getBounds().getWidth()) / 2.0);
							g2d.drawString(printData[page][0], dm, etH + 270);
//							etf = Font.decode("Free 3 of 9 Extended-PLAIN-60");
...
//									System.out.println("lbi = " + lbi);
									...
									layout = new TextLayout(printData[lbi][0], etf, frc);	//indeks katalogowy
									dm = (int) Math.round((etW - layout.getBounds().getWidth()) / 2.0);
									g2d.drawString(printData[lbi][0], dm + lm + j * etW, 40 + gm + i * etH);
									layout = new TextLayout(printData[lbi][0], etf, frc);	//indeks handlowy
									dm = (int) Math.round((etW - layout.getBounds().getWidth()) / 2.0);
									g2d.drawString(printData[lbi][0], dm + lm + j * etW, 65 + gm + i * etH);
									etf = Font.decode("Verdana-PLAIN-12");
									g2d.setFont(etf);
									layout = new TextLayout(printData[lbi][0], etf, frc);	//kod paskowy
									dm = (int) Math.round((etW - layout.getBounds().getWidth()) / 2.0);
									g2d.drawString(printData[lbi][0], dm + lm + j * etW, 165 + gm + i * etH);
//									etf = Font.decode("Free 3 of 9 Extended-PLAIN-38");
									etf = Font.decode("Code 128-PLAIN-68");
									g2d.setFont(etf);
									kodp = code128(printData[lbi][0]);//
...
//									g2d.drawString(kodp, dm + lm + j * etW, 125 + gm + i * etH);
						...
						if (formatka == 6 && page < printData.length)
							if (getSelectedPrinter().getName().contains("DYMO"))
							{
								...
								layout = new TextLayout(printData[page][0], etf, frc);	//indeks katalogowy
								dm = (int) Math.round((etW - layout.getBounds().getWidth()) / 2.0);
								g2d.drawString(printData[page][0], dm + lm, 32 + gm);
								layout = new TextLayout(printData[page][0], etf, frc);	//indeks handlowy
								dm = (int) Math.round((etW - layout.getBounds().getWidth()) / 2.0);
								g2d.drawString(printData[page][0], dm + lm, 49 + gm);
								etf = Font.decode("Verdana-PLAIN-10");
								g2d.setFont(etf);
								layout = new TextLayout(printData[page][0], etf, frc);	//kod paskowy
								dm = (int) Math.round((etW - layout.getBounds().getWidth()) / 2.0);
								g2d.drawString(printData[page][0], dm + lm, 128 + gm);
//								etf = Font.decode("Free 3 of 9 Extended-PLAIN-24");
								etf = Font.decode("Code 128-PLAIN-36");
								g2d.setFont(etf);
								kodp = code128(printData[page][0]);//"*" + printData[page][3] + "*";
...
//								g2d.drawString(printData[page][2], dm + lm, 34 + gm);
								etf = Font.decode("Verdana-PLAIN-25");
								g2d.setFont(etf);
								layout = new TextLayout(printData[page][0], etf, frc);	//kod paskowy
								dm = (int) Math.round((etW - layout.getBounds().getWidth()) / 2.0);
								g2d.drawString(printData[page][0], dm + lm, 92 + gm);
